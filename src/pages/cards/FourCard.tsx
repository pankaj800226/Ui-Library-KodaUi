...
const KodaBlogCard = () => {
  ...
  // Updated source code to include EVERYTHING the user needs
...
const ModernBlogCard = () => {
  ...
  return (
    <motion.div 
      whileHover={{ y: -8 }}
      className="w-full max-w-sm bg-[#09090b] border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl group"
    >
      {/* Image Section */}
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute top-5 left-5 px-4 py-1.5 bg-indigo-600 rounded-full text-[10px] font-black text-white uppercase tracking-widest cursor-pointer shadow-lg shadow-indigo-600/20">
          Design
        </div>
      </div>

      <div className="p-8">
        <div className="flex items-center gap-2 text-neutral-500 text-[10px] font-black uppercase tracking-widest mb-4">
          <FiClock className="text-indigo-500" /> 6 min read • Feb 5, 2026
        </div>
        
        <h3 className="text-2xl font-black text-white italic tracking-tighter leading-tight mb-4 group-hover:text-indigo-400 transition-colors cursor-pointer">
        ...
        <div className="flex items-center justify-between border-t border-white/5 pt-6">
          ...
          <button 
            ...
            className="p-3 bg-white/5 rounded-xl text-neutral-500 hover:text-red-500 transition-all cursor-pointer active:scale-90"
          >
    ...
    <div className="w-full max-w-5xl mx-auto my-12 bg-neutral-950 border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
      ...
      <div className="p-10 md:p-20 flex justify-center bg-[radial-gradient(circle_at_bottom_right,_var(--tw-gradient-stops))] from-indigo-900/20 via-transparent to-transparent">
        ...
        >
          <div className="relative h-56 overflow-hidden">
            ...
            <div className="absolute top-5 left-5 bg-indigo-600 text-white text-[10px] font-black px-4 py-1.5 rounded-full shadow-lg shadow-indigo-600/20 uppercase tracking-widest cursor-pointer">
              Design
            </div>
          </div>
      ...
      <div className="px-10 py-6 bg-white/5 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <p className="text-[10px] font-mono text-neutral-500 font-black uppercase tracking-[0.3em]">Module / Editorial_Card_v4</p>
        </div>

        <button
          onClick={handleCopy}
          className="w-full md:w-auto flex items-center justify-center gap-2 bg-white text-black px-10 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all active:scale-95 cursor-pointer shadow-xl"
        >
          {copied ? <><FiCheck /> SOURCE COPIED</> : <><FiCopy /> COPY FULL COMPONENT</>}
        </button>
      </div>
    </div>
  );
